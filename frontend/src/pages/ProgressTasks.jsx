...
import { Sparkles, Calendar, User, Search, RefreshCw, MessageSquare } from 'lucide-react';
...
export default function ProgressTasks() {
  ...
  const fetchProgressTasks = async () => {
    ...
    try {
      ...
      const progress = response.data.filter(t => t.status === 'In Progress' || t.status === 'Review');
      setTasks(progress);
    } catch (error) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 p-6 glass-panel rounded-3xl bg-gradient-to-r from-sky-500/10 to-transparent">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Displaying tasks that are actively being worked on or awaiting code reviews.
          </p>
        </div>
        <button 
      ...
      <div className="relative max-w-md">
        ...
        <input
          type="text"
          placeholder="Filter ongoing tasks..."
          value={search}
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filtered.map(task => (
            <div 
              ...
              className="glass-panel p-5 rounded-2xl flex flex-col border-t-4 border-t-sky-500 hover:translate-y-[-2px] hover:border-sky-500/30 transition-all duration-300 cursor-pointer text-left animate-float"
            >
              ...
              <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-[10px] font-semibold text-slate-400">
                <div className="flex items-center gap-2">
                  ...
                  {task.comments?.length > 0 && (
                    <span className="flex items-center gap-0.5 text-sky-500 font-bold"><MessageSquare className="h-3.5 w-3.5" /> {task.comments.length}</span>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <span className="px-1.5 py-0.5 bg-purple-500/10 border border-purple-500/20 text-purple-500 text-[8px] font-extrabold uppercase rounded">
                    {task.status}
                  </span>
                  <span className="flex items-center gap-1"><User className="h-3.5 w-3.5 text-sky-500" /> {task.assignee.split('_')[0]}</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="text-center py-16 bg-white/10 dark:bg-slate-900/10 rounded-2xl p-6">
          ...
          <h4 className="font-bold text-slate-700 dark:text-slate-300">No Ongoing Sprints Found</h4>
          <p className="text-xs text-slate-400 mt-1">There are no tasks currently in progress or review stages.</p>
        </div>
      )}
      {/* Task Details Modal */}
      {selectedTask && (
